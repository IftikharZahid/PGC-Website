...
const Admissions = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create admission object with flattened structure for admin panel
      const admissionData = {
        name: formData.fullName,
        fatherName: 'N/A', // Form doesn't have father name field yet
        email: formData.email,
        phone: formData.phone,
        course: formData.program.desiredCourse,
        previousMarks: formData.academic.gpa,
        gender: formData.gender,
        address: `${formData.address.street}, ${formData.address.city}`,

        // Keep original nested data just in case
      ...
      // In a real app, this would be: const response = await fetch('/api/admissions', ...)
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate delay

      const newAdmission = addItem(STORAGE_KEYS.ADMISSIONS, admissionData);

      // Check for system notifications setting
      const savedSettings = localStorage.getItem('admin_settings');
      if (savedSettings) {
        const settings = JSON.parse(savedSettings);
        if (settings.systemNotifications && Notification.permission === "granted") {
          // Get total count
          const allAdmissions = JSON.parse(localStorage.getItem(STORAGE_KEYS.ADMISSIONS) || '[]');
          const count = allAdmissions.length;

          new Notification("New Student Registered!", {
            body: `Total Registered Students: ${count}\nStudent: ${formData.fullName}`,
            icon: '/favicon.ico'
          });
        }
      }

      setSuccess(`Application submitted successfully! Your Application ID: ${newAdmission.id}`);
